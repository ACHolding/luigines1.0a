...
constexpr int kMenuBarH = 24;
constexpr int kStatusBarH = 24;
constexpr int kPixelScale = 2;
constexpr int kGameDstW = kNesWidth * kPixelScale;
constexpr int kGameDstH = kNesHeight * kPixelScale;
constexpr int kFrameBorder = 2;
constexpr int kWindowWidth = 48 + kFrameBorder * 2 + kGameDstW;
constexpr int kWindowHeight =
    kMenuBarH + 12 + kFrameBorder * 2 + kGameDstH + kStatusBarH;
...
int textWidth(TTF_Font *font, const char *utf8) {
  int w = 0, h = 0;
...
void drawTextRight(SDL_Renderer *ren, TTF_Font *font, const std::string &text,
                   int rightEdgeX, int y, SDL_Color color) {
  const int w = textWidth(font, text.c_str());
  drawText(ren, font, text, rightEdgeX - w, y, color);
}

void drawFullscreenCorners(SDL_Renderer *ren, SDL_Rect inner) {
  const int m = 5;
  const int L = 7;
  SDL_SetRenderDrawColor(ren, 240, 240, 245, 255);
  const int x0 = inner.x + inner.w - m - L;
  const int y0 = inner.y + inner.h - m - L;
  SDL_RenderDrawLine(ren, x0, y0, x0 + L, y0);
  SDL_RenderDrawLine(ren, x0, y0, x0, y0 + L);
  SDL_RenderDrawLine(ren, x0 + L, y0 + L, x0, y0 + L);
  SDL_RenderDrawLine(ren, x0 + L, y0 + L, x0 + L, y0);
}
...
int main(int argc, char **argv) {
  ...
  SDL_Window *window = SDL_CreateWindow(
      "Luigines 1.X", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
  ...
  NesCore nes;
  bool running = false;
  std::string status = "Load ROM: File menu, drag/drop, or Cmd/Ctrl+O";
  if (argc > 1) {
  ...
  Uint64 fpsT0 = SDL_GetPerformanceCounter();
  int fpsFrames = 0;
  ...
  while (!quit) {
    ...
    const int availH = wh - kMenuBarH - kStatusBarH;
    int scale = std::min(ww / kNesWidth, availH / kNesHeight);
    if (scale < 1)
      scale = 1;
    const int gw = kNesWidth * scale;
    const int gh = kNesHeight * scale;
    const int gx = (ww - gw - 2 * kFrameBorder) / 2;
    const int gy = kMenuBarH + (availH - gh - 2 * kFrameBorder) / 2;
    const SDL_Rect gameOuter{gx, gy, gw + 2 * kFrameBorder, gh + 2 * kFrameBorder};
    const SDL_Rect gameInner{gx + kFrameBorder, gy + kFrameBorder, gw, gh};

    const int mx0 = 8;
    SDL_Rect rFile{mx0, 0, textWidth(font, "File") + 16, kMenuBarH};
    int x1 = rFile.x + rFile.w;
    SDL_Rect rCpu{x1, 0, textWidth(font, "CPU") + 16, kMenuBarH};
    x1 = rCpu.x + rCpu.w;
    SDL_Rect rOpt{x1, 0, textWidth(font, "Options") + 16, kMenuBarH};
    x1 = rOpt.x + rOpt.w;
    SDL_Rect rHelp{x1, 0, textWidth(font, "Help") + 16, kMenuBarH};

    const SDL_Rect fsHit{gameInner.x + gameInner.w - 30,
                         gameInner.y + gameInner.h - 26, 28, 24};

    const SDL_Rect filePanel{rFile.x, kMenuBarH, 200, 52};
    const SDL_Rect fileOpen{filePanel.x, filePanel.y, filePanel.w, 26};
    const SDL_Rect fileQuit{filePanel.x, filePanel.y + 26, filePanel.w, 26};

    const SDL_Rect cpuPanel{rCpu.x, kMenuBarH, 200, 52};
    const SDL_Rect cpuPause{cpuPanel.x, cpuPanel.y, cpuPanel.w, 26};
    const SDL_Rect cpuReset{cpuPanel.x, cpuPanel.y + 26, cpuPanel.w, 26};

    const SDL_Rect optPanel{rOpt.x, kMenuBarH, 220, 28};

    const SDL_Rect helpPanel{rHelp.x, kMenuBarH, 300, 108};
    ...
    while (SDL_PollEvent(&e)) {
      ...
      if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
        ...
        if (inside(fsHit, mx, my)) {
          fullscreen = !fullscreen;
          SDL_SetWindowFullscreen(
              window, fullscreen ? SDL_WINDOW_FULLSCREEN_DESKTOP : 0);
        } else if (inside(rFile, mx, my)) {
          openMenu = (openMenu == 1) ? 0 : 1;
        } else if (inside(rCpu, mx, my)) {
          openMenu = (openMenu == 2) ? 0 : 2;
        } else if (inside(rOpt, mx, my)) {
          openMenu = (openMenu == 3) ? 0 : 3;
        } else if (inside(rHelp, mx, my)) {
          openMenu = (openMenu == 4) ? 0 : 4;
        } else if (openMenu == 1) {
          if (inside(fileOpen, mx, my)) {
            const std::string path = pickRomOnMac();
            if (!path.empty() && nes.loadRom(path)) {
              running = true;
              status =
                  "Loaded m" + std::to_string(nes.mapperId()) + ": " + path;
            } else {
              status = "Open ROM canceled or failed";
            }
            openMenu = 0;
          } else if (inside(fileQuit, mx, my)) {
            quit = true;
          } else {
            openMenu = 0;
          }
        } else if (openMenu == 2) {
          if (inside(cpuPause, mx, my)) {
            running = !running;
            status = running ? "Running" : "Paused";
            openMenu = 0;
          } else if (inside(cpuReset, mx, my)) {
            if (nes.loaded()) {
              nes.reset();
              status = "Core reset";
            } else {
    ...
    const double el =
        static_cast<double>(t1 - fpsT0) / SDL_GetPerformanceFrequency();
    if (el >= 0.5) {
      fpsValue = fpsFrames / el;
      fpsFrames = 0;
      fpsT0 = t1;
    }
    ...
    SDL_Rect menuBg{0, 0, ww, kMenuBarH};
    SDL_SetRenderDrawColor(ren, 236, 236, 240, 255);
    SDL_RenderFillRect(ren, &menuBg);
    SDL_SetRenderDrawColor(ren, 190, 190, 196, 255);
    SDL_RenderDrawLine(ren, 0, kMenuBarH - 1, ww, kMenuBarH - 1);

    drawText(ren, font, "File", rFile.x + 8, 4, SDL_Color{28, 28, 32, 255});
    drawText(ren, font, "CPU", rCpu.x + 8, 4, SDL_Color{28, 28, 32, 255});
    drawText(ren, font, "Options", rOpt.x + 8, 4, SDL_Color{28, 28, 32, 255});
    drawText(ren, font, "Help", rHelp.x + 8, 4, SDL_Color{28, 28, 32, 255});

    SDL_Rect body{0, kMenuBarH, ww, wh - kMenuBarH - kStatusBarH};
    SDL_SetRenderDrawColor(ren, 252, 252, 254, 255);
    SDL_RenderFillRect(ren, &body);

    SDL_SetRenderDrawColor(ren, 0, 0, 0, 255);
    SDL_RenderFillRect(ren, &gameOuter);
    SDL_SetRenderDrawColor(ren, 188, 188, 194, 255);
    SDL_RenderDrawRect(ren, &gameOuter);

    SDL_RenderCopy(ren, frameTex, nullptr, &gameInner);
    drawFullscreenCorners(ren, gameInner);

    if (openMenu == 1) {
      SDL_SetRenderDrawColor(ren, 255, 255, 255, 255);
      SDL_RenderFillRect(ren, &filePanel);
      SDL_SetRenderDrawColor(ren, 160, 160, 170, 255);
      SDL_RenderDrawRect(ren, &filePanel);
      SDL_RenderDrawLine(ren, filePanel.x, filePanel.y + 26,
                         filePanel.x + filePanel.w, filePanel.y + 26);
      drawText(ren, font, "Open ROM...", filePanel.x + 10, filePanel.y + 5,
               SDL_Color{20, 20, 24, 255});
      drawText(ren, font, "Quit", filePanel.x + 10, filePanel.y + 31,
               SDL_Color{20, 20, 24, 255});
    } else if (openMenu == 2) {
      SDL_SetRenderDrawColor(ren, 255, 255, 255, 255);
      SDL_RenderFillRect(ren, &cpuPanel);
      SDL_SetRenderDrawColor(ren, 160, 160, 170, 255);
      SDL_RenderDrawRect(ren, &cpuPanel);
      SDL_RenderDrawLine(ren, cpuPanel.x, cpuPanel.y + 26,
                         cpuPanel.x + cpuPanel.w, cpuPanel.y + 26);
      drawText(ren, font, "Run / Pause", cpuPanel.x + 10, cpuPanel.y + 5,
               SDL_Color{20, 20, 24, 255});
      drawText(ren, font, "Reset", cpuPanel.x + 10, cpuPanel.y + 31,
               SDL_Color{20, 20, 24, 255});
    } else if (openMenu == 3) {
      SDL_SetRenderDrawColor(ren, 255, 255, 255, 255);
      SDL_RenderFillRect(ren, &optPanel);
      SDL_SetRenderDrawColor(ren, 160, 160, 170, 255);
      SDL_RenderDrawRect(ren, &optPanel);
      drawText(ren, font, "VSync (always on)", optPanel.x + 10, optPanel.y + 6,
               SDL_Color{20, 20, 24, 255});
    } else if (openMenu == 4) {
      SDL_SetRenderDrawColor(ren, 255, 255, 255, 255);
      SDL_RenderFillRect(ren, &helpPanel);
      SDL_SetRenderDrawColor(ren, 160, 160, 170, 255);
      SDL_RenderDrawRect(ren, &helpPanel);
      const int hx = helpPanel.x + 10;
      int hy = helpPanel.y + 6;
      drawText(ren, font, "A: X   B: Z", hx, hy, SDL_Color{20, 20, 24, 255});
      hy += 18;
      drawText(ren, font, "Select: RShift   Start: Enter", hx, hy,
               SDL_Color{20, 20, 24, 255});
      hy += 18;
      drawText(ren, font, "D-Pad: Arrow keys", hx, hy,
               SDL_Color{20, 20, 24, 255});
      hy += 18;
      drawText(ren, font, "Open ROM: Cmd/Ctrl+O or File menu", hx, hy,
               SDL_Color{20, 20, 24, 255});
      hy += 18;
      drawText(ren, font, "Click outside to close", hx, hy,
               SDL_Color{60, 60, 70, 255});
    }

    SDL_Rect statusBg{0, wh - kStatusBarH, ww, kStatusBarH};
    SDL_SetRenderDrawColor(ren, 232, 232, 236, 255);
    SDL_RenderFillRect(ren, &statusBg);
    SDL_SetRenderDrawColor(ren, 190, 190, 196, 255);
    SDL_RenderDrawLine(ren, 0, wh - kStatusBarH, ww, wh - kStatusBarH);

    drawText(ren, font, status, 10, wh - kStatusBarH + 4,
             SDL_Color{40, 40, 48, 255});
    const std::string fpsStr =
        std::to_string(static_cast<int>(fpsValue + 0.5)) + " fps";
    drawTextRight(ren, font, fpsStr, ww - 10, wh - kStatusBarH + 4,
                  SDL_Color{40, 40, 48, 255});
